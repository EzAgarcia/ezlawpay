...
$(document).ready(function() {
  ...
  $('#contracts').DataTable( {
    ...
    "serverSide": true,
    "ajax": {
      url: '<?php echo base_url()?>' + 'contract_table_list2',
      type: 'POST',
      data: default_data
    },
    "columnDefs": [ {
    ...
    //   }},
      ...
      "data": null,
      "render": function ( data, type, row, meta ) {
        return '<div style="cursor: pointer" class="ccolor" data-toggle="modal" data-target="#general_modal" onclick="showContract(' + row[4] +');">' + row[1] +'</div>'
      }
    //   {
    //   "targets": 3,
    //   "data": null,
...
$(document).ready(function() {
  ...
  function reload(raw_month, raw_year, raw_status){
    ...
    raw_url = '<?php echo base_url()?>' + 'monthly_bills?month=' + month + '&year=' + year;
...
function edit_tools(contract, balance_status) {
  ...
  return button + button_style + menu + edit + update_amount + send_balance_link + reset_password + delete_contract + close_button
}
...
function showContract(ok){
  $.ajax({
    data: {ok: ok},
    url: "<?php echo site_url('C_contracts/showbills');?>",
    type:"POST",
    dataType:"html",
  }).done(function(data) {
    $(".modal-body").html(data);
