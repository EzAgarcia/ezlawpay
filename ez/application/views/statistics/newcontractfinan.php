  ...
  <div class="container">
    <div class="row">
      <div class="col-3">
        <div class="vertical-menu">
          ...
          <a  class="active" href="<?php echo base_url('C_contracts/newcontractsf')?>">Contract Value</a>
    ...
    <!--  <button  style="margin-top: .5rem; margin-left: .8rem;" class="okss btn btn-info" onclick="exceltype()">Export to Excel</button> -->
     ...
     <button  style="margin-top: .5rem; margin-left: .8rem;" class="okss btn btn-info" onclick="excel()">Report</button>
        ...
        <center><h4 id="promedio"></h4></center>
    ...
    <br>
      ...
      <div class="col-md-4">
        <ul class="list-group" id="prueba">
          <li style="background-color: #007bff; color: #FFFFFF; height: 45px; margin-top:15px;" class="list-group-item"></li>
        </ul>
      </div>
...
<script>
  ...
  $( document ).ready(function() {
    ...
    $.ajax({
      url: "<?php echo site_url('C_contracts/contractsfinancial');?>",
      type: 'POST',
    ...
    options: {
      ...
      scales: {
          ...
          for (var i = 0; i < 10; i++) {
            ...
            $("#prueba").append('<li  class="list-group-item d-flex justify-content-between align-items-center">'+fechas[i]+'<span onclick=elim() class="badge badge-primary badge-pill">'+ "$" + new Intl.NumberFormat().format(total[i])+'</span></li>');
          }
          for (var i = 10; i < 20; i++) {
            ...
            $("#prueba1").append('<li  class="list-group-item d-flex justify-content-between align-items-center">'+fechas[i]+'<span onclick=elim() class="badge badge-primary badge-pill">'+"$" + new Intl.NumberFormat().format(total[i])+'</span></li>');
          }
          ...
          for (var i = 20; i < fechas.length; i++) {
            ...
            $("#prueba2").append('<li  class="list-group-item d-flex justify-content-between align-items-center">'+fechas[i]+'<span onclick=elim() class="badge badge-primary badge-pill">'+"$" + new Intl.NumberFormat().format(total[i])+'</span></li>');
          }
    ...
    $.ajax({
      url: "<?php echo site_url('C_contracts/totalfinancial');?>",
      type: 'GET',
      success:function(response){

          
          $("#total").text("Total value of contracts : $"+ new Intl.NumberFormat().format(response));
   

      }});

    $.ajax({
      url: "<?php echo site_url('C_contracts/totalfinanpromedio');?>",
      type: 'GET',
      success:function(response){

          var promedio = Number.parseFloat((parseInt(response, 10))).toFixed(2);
          $("#promedio").text("Average : $"+ new Intl.NumberFormat().format(promedio));
...
function excel(){
  ...
  window.location.href = "https://ezlawpay.com/ez/C_reports/download_financial?mes="+mes+"&year="+year+"";
